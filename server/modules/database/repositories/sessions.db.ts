...
export const sessionsDb = {
   ...
   *
   * Primary intention: a provider can expose a new session in its shared
   * artifact store before the websocket runtime reports that same provider
   * id back to our app. At that moment the sidebar already has an optimistic
  ...
  /**
   * Lists every session row for one provider, including archived ones.
   *
   * Used by the per-provider reset flow so each session's abort fn can
   * be called before the row is dropped. Returns an empty array when
   * the provider has no rows.
   */
  getAllByProvider(provider: string): SessionRow[] {
    const db = getConnection();
    const rows = db
      .prepare(
        `SELECT ${SESSION_ROW_COLUMNS}
  ...
  /**
   * Deletes every session row for the given provider, atomically.
   *
