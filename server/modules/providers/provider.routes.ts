...
import { providerResetService } from '@/modules/providers/services/provider-reset.service.js';
 ...
 * overrides. Native CLI files under `~/.cursor/` (etc.) are intentionally
 * not touched — that is the user's responsibility or a future opt-in flow.
 */
router.post(
  '/:provider/reset',
  asyncHandler(async (req: Request, res: Response) => {
    const provider = parseProvider(req.params.provider);
    const summary = await providerResetService.resetProvider(provider);
    res.json(createApiSuccessResponse(summary));
  }),
);
