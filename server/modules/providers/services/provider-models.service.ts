...
export const createProviderModelsService = (dependencies: ProviderModelsServiceDependencies = {}) => {
  ...
  /**
   * Drops one provider from both the in-memory cache and the persisted
   * `provider-models-cache.json` file.
   *
   * Safe to call when the provider has no entry — returns `false` so
   * callers can report without distinguishing "missing" from "removed".
   * Filesystem failures are logged and re-thrown so the reset service
   * can surface them, but the in-memory entries are still cleared.
   */
  const clearCacheForProvider = async (provider: LLMProvider): Promise<boolean> => {
    memoryCache.delete(provider);
    pendingRequests.delete(provider);

    const cacheFile = await readProviderModelsCacheFile(cachePath);
    if (!cacheFile) {
      // Persist an empty cache file so subsequent loads don't trip the
      // "missing file → fresh load" path on the next read.
      await writeProviderModelsCacheFile(cachePath, new Map(), now());
      return false;
    }

    if (!(provider in cacheFile.entries)) {
      return false;
    }

    delete cacheFile.entries[provider];
    const remaining = new Map<LLMProvider, ProviderModelsCacheEntry>();
    for (const [key, value] of Object.entries(cacheFile.entries)) {
      remaining.set(key as LLMProvider, value);
    }
    await writeProviderModelsCacheFile(cachePath, remaining, now());
    return true;
  };
