 ...
 * The function resolves symlinks and returns a normalized absolute path. No
 * path is rejected — workspaces may be created under any directory on disk.
 */
...
/**
 * Removes every persisted session-model-change entry whose key starts with
 * `${provider}:` from the cache file.
 *
 * Missing or unreadable files are treated as already empty (returns 0).
 * Used by the per-provider reset flow so the cache does not keep stale
 * "resume with this model" entries for sessions that no longer exist.
 */
export async function clearProviderSessionActiveModelChanges(
  provider: LLMProvider,
 ...
 *
 * Used by the Claude spawn runtime.
 */
